...
def segmented_one_sided_desirability(y, y_min, y_max, y_mid, r1=1, r2=1):
    """
    ...
    if y < y_min:
        ...
        return 0.5 * ((y - y_min) / (y_mid - y_min))**r1
    else:
        # Second segment scaling (y_mid to y_max)
        return 0.5 + 0.5 * ((y - y_mid) / (y_max - y_mid))**r2

def segmented_negative_one_sided_desirability(d, d_min, d_max, d_mid, r1=1, r2=1):
    """
    Segmented desirability function for cases where smaller values are better,
    with a middle point that divides the range into two segments.
    Based on the work of In-Jun Jeong and Kwang-Jae Kim (2006).
    ...
    d_mid (float): Middle point that divides the range into two segments
    r1 (float): Shape parameter for first segment (default=1 for linear)
    r2 (float): Shape parameter for second segment (default=1 for linear)
    ...
    if d < d_min:
        ...
        return 0.5 + 0.5 * (1-(d - d_min) / (d_mid - d_min))**r1
    else:
        # Second segment scaling (d_mid to d_max)
        return 0.5 - 0.5 * ((d - d_mid) / (d_max - d_mid))**r2
